...
def _split_time_parts(text: str) -> List[str]:
    if not text:
        return []
    cleaned = re.sub(r"\s+", " ", text)
    parts = re.split(
        r"\s*(?:-|–|—|to|until|til|thru|through|&ndash;|&mdash;)\s*",
        cleaned,
        flags=re.IGNORECASE,
    )
    out: List[str] = []
    for part in parts:
        ...
        if not part:
            continue
        lower = part.lower()
        if "all day" in lower or "all-day" in lower:
            continue
        out.append(part)
    return out
...
def _date_time_from_fields(fields: Dict[str, str], tz_name: Optional[str], *candidates: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    for candidate in lowered:
        ...
        prefer_end = base.startswith("end") or candidate.startswith("end")

        if prefer_end and not date_text:
            for key in ("startdate", "start_date", "start-date"):
                if key in fields:
                    date_text = fields[key]
                    break

        if prefer_end and not time_text:
            for key in ("endtime", "end_time", "end-time", "starttime", "start_time", "start-time"):
                if key in fields:
                    time_text = fields[key]
                    break

        time_source = time_text
        if time_text:
            parts = _split_time_parts(time_text)
            if parts:
                time_text = parts[-1] if prefer_end else parts[0]
            else:
                time_text = None
        ...
        if date_text:
            ...
            if dt:
                return dt, time_source or combined
